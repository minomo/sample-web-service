...
class DeviceHandler(webapp2.RequestHandler):
        ...
        if urlsafe:
            ...
            if entity:
                ...
                template_path = os.path.join(
                    os.path.dirname(__file__), 'templates', 'device.html')
    ...
    def post(self, urlsafe):
        ...
        lon = self.request.get('lon') or self.request.get(
            'lng') or self.request.get('longitude')
...
class RegisterDevice(register_handler.RegisterHandler):
    ...
    def _add(self, user):
        ...
        shortUrl = helpers.ShortenUrl(
            self.request.host_url + BASE_URL + key.urlsafe())
        if shortUrl:
            entity.url = shortUrl
            entity.put()
